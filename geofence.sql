...
CREATE TRIGGER trig_readings_before_insert BEFORE INSERT ON readings FOR EACH ROW BEGIN
	...
	DECLARE accountID int;
		...
		IF new_violation_count>0 THEN
            ...
            #Do not create exit event if it already exists in the future because of out of order readings
            IF NOT EXISTS( SELECT * FROM readings WHERE created_at>NEW.created_at AND geofence_id=first_fence_id AND geofence_event_type='exit' ) THEN
                #insert 1 new violation into geofence_violations table
                INSERT INTO geofence_violations (geofence_id, device_id, violation_time) SELECT fence_id, NEW.device_id, NEW.created_at FROM temp_violated_fences limit 1;
                SET NEW.geofence_event_type = 'enter';
                SET NEW.geofence_id = first_fence_id;
                SET proceed = false;
            END IF;
		END IF; 
		...
		IF proceed=true THEN
			...
			INSERT INTO temp_exited_fences SELECT g.id, g.fence_num FROM geofence_violations v, geofences g 
		    	...
		    	AND v.device_id=NEW.device_id AND v.violation_time<NEW.created_at;
			...
			IF geofence_exit_count > 0 THEN
		  		...
		  		SET NEW.geofence_event_type = 'exit';
		  		SET NEW.geofence_id = first_fence_id;
