...
namespace :FAST do
  ...
  task :create_new_branch do
    ...
    svn_base = DeployManagerClient.get_repo("coretracking")
    check = execute_cmd("svn info #{svn_base}/branches/#{branch_name}")
    if(check.length==0)
      execute_cmd("svn mkdir https://svn.numerexfast.com/repos/fast/Ublip_v2/branches/#{branch_name} -m \"creating new customer branch\"")
      execute_cmd("svn mkdir https://svn.numerexfast.com/repos/fast/Ublip_v2/branches/#{branch_name}/tags -m 'creating new customer branch'")
  ...
  desc 'Update customer\'s production and staging branch to latest from trunk'
  task :update_production_branch do
     ...
     staging_url = "#{cust_svn_base}/tags/current_staging_build"
     cust_trunk = "#{cust_svn_base}/trunk"
     execute_cmd("svn delete #{prod_url} -m 'removing previous production build'")
     execute_cmd("svn delete #{staging_url} -m 'removing previous staging build'")
     execute_cmd("svn delete #{cust_trunk} -m 'removing previous trunk'")
     ...
     execute_cmd("svn copy #{svn_base}/tags/current_production_build #{staging_url} -m \"updating branch to latest prod build from trunk\"")
  end
  ...
  task :update_staging_branch do
     ...
     execute_cmd("svn copy #{svn_base}/tags/current_staging_build #{staging_url} -m \"updating branch to latest staging build from trunk\"")
  end
...
def get_revision_datetime(rev)
  ...
  svn_info.each_line do |line|
    if (line.include?("Last Changed Date:")) 
      ...
      datetime_utc = datetime.new_offset(0)
