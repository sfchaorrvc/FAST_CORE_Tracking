...
class RecentReadingTriggerTest < ActiveSupport::TestCase
  ...
  context "an old reading inserted" do
    setup do
      without_timestamping_of(Reading) do
        @device = Factory.create(:device)
        @reading = Factory.create(:reading, :device => @device, :created_at => Time.now)
