...
class UserTest < ActiveSupport::TestCase
  ...
  def test_remember_token
    ...
    stub(Time).now {Time.at(1185490000+172800)} #two days later
    assert_equal false, user.remember_token?
  end

  should "paginate search_for_users" do
    def self.search_for_users(params, page)
      by_last_name.search(params).paginate(:page => page)
    end
    
    mock(User).by_last_name{ mock!.search("params") { mock!.paginate(:page => "1"){ WillPaginate::Collection.new(1,1,1)}}}
    User.search_for_users("params", "1")
  end
